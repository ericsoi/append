...
if(ISSET($_POST['save'])){
    ...
    if ($_POST["status"]=="Complete Loans"){
        ...
        $sum_payment=$db->conn->query("SELECT SUM(paid_amount), SUM(totalAmount), SUM(amount), count(*) FROM loan_test WHERE CAST(paid_amount AS DECIMAL) >= CAST(totalAmount AS DECIMAL) AND date_released >= CAST('$startDate' AS DATE) AND date_released <= CAST('$endDate' AS DATE)");
